...
def analyze_service_principals(tenant_mode="auto"):
    ...
    try:
        ...
        # Expired SPs
        cursor.execute("SELECT COUNT(*) FROM service_principals WHERE expired_credentials = 1")
        ...
        cursor.execute("SELECT COUNT(*) FROM service_principals WHERE has_credentials = 0")
        sps_no_credentials = cursor.fetchone()[0]
        
        # Apps with credentials (breakdown of expired vs non-expired)
        cursor.execute("SELECT COUNT(*) FROM service_principals WHERE has_credentials = 1")
        apps_with_credentials = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM service_principals WHERE has_credentials = 1 AND expired_credentials = 1")
        apps_with_expired_credentials = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM service_principals WHERE has_credentials = 1 AND expired_credentials = 0")
        ...
        expired_of_credentialed_percentage = (apps_with_expired_credentials / apps_with_credentials * 100) if apps_with_credentials > 0 else 0

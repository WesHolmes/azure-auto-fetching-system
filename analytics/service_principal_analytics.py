...
def analyze_service_principals(tenant_mode="auto"):
    ...
    try:
        ...
        # Expired SPs
        cursor.execute(
            "SELECT COUNT(*) FROM service_principals WHERE credential_exp_date IS NOT NULL AND credential_exp_date < datetime('now')"
        )
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM service_principals WHERE credential_type IS NOT NULL AND credential_exp_date IS NOT NULL AND credential_exp_date < datetime('now')"
        )
        apps_with_expired_credentials = cursor.fetchone()[0]

        cursor.execute(
            "SELECT COUNT(*) FROM service_principals WHERE credential_type IS NOT NULL AND (credential_exp_date IS NULL OR credential_exp_date >= datetime('now'))"
        )

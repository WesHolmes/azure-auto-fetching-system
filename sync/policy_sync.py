...
def resolve_policy_applications(tenant_id, policy, tenant_applications):
    ...
    logger.info(f"Processing policy '{policy_name}' - includeApplications: {include_applications}, excludeApplications: {list(exclude_applications)}")
...
def sync_conditional_access_policies(tenant_id, tenant_name):
    ...
    try:
        ...
        logger.info(f"Successfully synced {policies_stored} conditional access policies, {policy_users_stored} user assignments, and {policy_applications_stored} application assignments for {tenant_name}")

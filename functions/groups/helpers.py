...
def fetch_group_members(tenant_id, group_id, use_beta=True):
...
def fetch_group_owners(tenant_id, group_id, use_beta=True):
    """Fetch owners of a specific group"""
    try:
        if use_beta:
            graph = GraphBetaClient(tenant_id)
        else:
...
def transform_group_data(groups, tenant_id, use_beta=True):
    ...
    try:
        ...
        for group in groups:
            ...
            members = fetch_group_members(tenant_id, group.get("id"), use_beta)
            owners = fetch_group_owners(tenant_id, group.get("id"), use_beta)

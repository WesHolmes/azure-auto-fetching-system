...
class HIBPClient:
    ...
    def _get_next_available_key(self) -> Optional[str]:
        ...
        self.key_rate_limited_until = {
            key: until
            for key, until in self.key_rate_limited_until.items()
        ...
        if self.key_rate_limited_until:
            soonest_key = min(self.key_rate_limited_until.items(), key=lambda x: x[1])[
                0
            ]
    ...
    def check_email_breaches(self, email: str) -> Optional[List[Dict]]:
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"Rate limited on key {self.current_key_index + 1}, switching keys"
                    )
